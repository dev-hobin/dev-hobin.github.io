...
const iconBaseStyles = {
  ...
  "&:after": {
    ...
    boxShadow: (t: Theme) =>
      `0 -23px 0 ${get(t, `colors.toggleIcon`)}, 0 23px 0 ${get(
        t,
        `colors.toggleIcon`
      )}, 23px 0 0 ${get(t, `colors.toggleIcon`)}, -23px 0 0 ${get(
        t,
        `colors.toggleIcon`
    ...
    <TogglePrimitive fallback={<FallbackIcon />}>
      <button
        onClick={() => {
          ...
          if (isDark) {
            htmlElement.classList.remove("theme-ui-dark");
            htmlElement.classList.add("theme-ui-light");
          } else {
            htmlElement.classList.remove("theme-ui-light");
            htmlElement.classList.add("theme-ui-dark");
          }
        }}
        type="button"
      ...
      >
        <div
          sx={{
            ...
            boxShadow: (t) =>
              isDark ? `none` : `inset 8px -8px 0px 0px ${get(t, `colors.toggleIcon`)}`,
            "&:before": {
              ...iconBaseStyles[`&:before`],
